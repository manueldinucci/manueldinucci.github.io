...
add('a21|club','Douvikas','A','S2',45,55,70); add('a22|club','Scamacca','A','S2',45,55,69); add('a23|club','Davis K.','A','S2',35,45,68); add('a24|club','Simeone','A','S2',35,45,67); add('a25|club','Berardi','A','S2',30,40,66); add('a26|club','Dybala','A','S2',30,40,65)
...
with sync_playwright() as pw:
    ...
    for width in (390,360):
        ...
        open_map(page,'A')
        if width == 390:
            page.locator('#slotMapSheet').screenshot(path=str(ROOT/'tests/v32-8-map-att.png'))
        ...
        # v32.8: a semantic new-microtier row has the names-only divider and vertical rhythm matching vertical bands.
        semantic_rows=page.locator('[data-slot-map-slot="S1"] .slot-map-inline-row.microtier-start')
        check(semantic_rows.count()>=2,f'A fixture needs >=2 semantic rows at {width}')
        second_sem=semantic_rows.nth(1)
        compact_names=second_sem.locator('.slot-map-inline')
        before=compact_names.evaluate("e=>{const s=getComputedStyle(e,'::before');return {content:s.content,left:s.left,right:s.right,height:s.height,bg:s.backgroundColor,pointer:s.pointerEvents}}")
        check(before['content'] not in ('none','normal',''),f'compact divider missing at {width}: {before}')
        check(before['left']=='0px' and before['right']=='0px',f'compact divider must span names column only at {width}: {before}')
        check(before['height']=='1px',f'compact divider height wrong at {width}: {before}')
        check(before['pointer']=='none',f'compact divider intercepts pointers at {width}: {before}')
        check(before['bg'] in ('rgba(0, 0, 0, 0.05)','rgba(0, 0, 0, 0.047)','rgba(0, 0, 0, 0.051)'),f'compact divider is not ultra-light at {width}: {before}')

        # Compare baseline-to-baseline rhythm: compact semantic rows vs S2 vertical microtiers.
        compact_baselines=semantic_rows.evaluate_all("rows=>rows.slice(0,2).map(r=>r.querySelector('.slot-map-player').getBoundingClientRect().top)")
        vb=page.locator('[data-slot-map-slot="S2"] .slot-map-band')
        check(vb.count()>=1,'A S2 fixture unexpectedly lacks vertical bands')

        check(vb.count()>=3,f'A S2 should have 3 vertical microtiers at {width}')
        second_v_names=vb.nth(1).locator('.slot-map-names')
        vbefore=second_v_names.evaluate("e=>{const s=getComputedStyle(e,'::before');return {content:s.content,left:s.left,right:s.right,height:s.height,bg:s.backgroundColor,pointer:s.pointerEvents}}")
        check(vbefore['content'] not in ('none','normal',''),f'vertical divider missing at {width}: {vbefore}')
        check(vbefore['left']=='0px' and vbefore['right']=='0px' and vbefore['height']=='1px',f'vertical divider geometry wrong at {width}: {vbefore}')
        check(vbefore['pointer']=='none',f'vertical divider intercepts pointers at {width}')
        # First microtier has no divider; selector must not create one.
        first_before=vb.nth(0).locator('.slot-map-names').evaluate("e=>getComputedStyle(e,'::before').content")
        check(first_before in ('none','normal',''),f'first vertical microtier unexpectedly has divider at {width}: {first_before}')
        # Effective row rhythm should be visually equivalent (top delta within 1px).
        vtops=vb.evaluate_all("rows=>rows.slice(0,2).map(r=>r.querySelector('.slot-map-player').getBoundingClientRect().top)")
        compact_delta=compact_baselines[1]-compact_baselines[0]
        vertical_delta=vtops[1]-vtops[0]
        check(abs(compact_delta-vertical_delta)<=1.0,f'compact/vertical microtier rhythm differs at {width}: {compact_delta} vs {vertical_delta}')
        ...
        continuation=page.locator('[data-slot-map-slot="S1"] .slot-map-inline-row.microtier-continuation').first
        cont_before=continuation.locator('.slot-map-inline').evaluate("e=>getComputedStyle(e,'::before').content")
        check(cont_before in ('none','normal',''),f'same-microtier continuation received divider at {width}: {cont_before}')
        check(float(continuation.evaluate("e=>parseFloat(getComputedStyle(e).marginTop)||0"))==0,f'same-microtier continuation received extra spacing at {width}')
        ...
        page.close()
    print('v32.8 runtime Playwright checks: OK')
    browser.close()
