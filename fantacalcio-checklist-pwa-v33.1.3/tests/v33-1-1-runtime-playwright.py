...
with sync_playwright() as pw:
    ...
    for width in (390,360):
        ...
        check(page.locator('label', has=page.locator('#targetMaxFilter')).inner_text().split('—')[0].strip().startswith('Target max ≥'), 'Target max ≥ label wrong')
        page.screenshot(path=str(ROOT/'tests'/f'v33-1-1-panel-{width}.png'), full_page=False)

        # Visual: all three checks are on one row and occupy three regular columns.
        labels=[page.locator('#onlyAvailable').locator('xpath=..').bounding_box(), page.locator('#onlyFavorites').locator('xpath=..').bounding_box(), page.locator('#onlyOneCredit').locator('xpath=..').bounding_box()]
        ...
        check(all(labels) and all(boxes), 'checkbox geometry missing')
        ys=[round(b['y']) for b in boxes]
        check(max(ys)-min(ys) <= 2, f'checkboxes not on same row at {width}: {ys}')
        centers=[b['x']+b['width']/2 for b in labels]
        gaps=[centers[1]-centers[0], centers[2]-centers[1]]
        check(abs(gaps[0]-gaps[1]) <= 8, f'checkbox distribution not regular at {width}: {gaps}')
        ...
        page.locator('#targetMaxFilter').select_option('20'); page.wait_for_timeout(30)
        check([x.inner_text() for x in page.locator('.player-name').all()] == ['Alpha','Beta'], 'target max >= wrong')
        check(page.locator('#visiblePlayerCount').inner_text() == '2 / 5', 'target count wrong')

        # Search changes numerator only. Reset filters first so the search assertion is isolated.
        page.locator('#resetFiltersBtn').click(); page.wait_for_timeout(20)
        ...
        page.screenshot(path=str(ROOT/'tests'/f'v33-1-1-filters-{width}.png'), full_page=False)
        check(not errors, f'page errors after interactions at {width}: {errors}')
        page.close()
    browser.close()
    print('v33.1.1 runtime: OK')
