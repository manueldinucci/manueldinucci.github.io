...
if (!/fantacalcio-checklist-v32\.[12345]/.test(sw)) assert(!app.includes('data-slot-map-player-key='), 'slot-map names must not be clickable before v32.1');
if (!/fantacalcio-checklist-v32\.[12345]/.test(sw)) assert(!app.includes("querySelectorAll('[data-slot-map-player-key]')"), 'slot-map click binding must be removed before v32.1');
...
if (!/fantacalcio-checklist-v32\.[12345]/.test(sw)) assert(!css.includes('cursor: pointer'), 'slot-map player pointer styling remains before v32.1');
