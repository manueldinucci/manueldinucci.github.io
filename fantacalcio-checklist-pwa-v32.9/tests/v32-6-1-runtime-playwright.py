...
with sync_playwright() as pw:
    ...
    # v32.6.1: Target and the first names line share the same typographic baseline.
    baseline_deltas=[]
    for slot in ('S3','S4','S5'):
        bands=page.locator(f'[data-slot-map-slot="{slot}"] .slot-map-band')
        for i in range(min(bands.count(),4)):
            band=bands.nth(i)
            delta=band.evaluate("""e => {
              const label=e.querySelector('.slot-map-band-label');
              const names=e.querySelector('.slot-map-names');
              ...
              const a=mk(), b=mk(); label.prepend(a); names.prepend(b);
              const d=Math.abs(a.getBoundingClientRect().top-b.getBoundingClientRect().top);
              a.remove(); b.remove(); return d;
            }""")
            baseline_deltas.append(delta)
    check(max(baseline_deltas)<=0.75,f'Target/name first-line baselines differ: {baseline_deltas}')
    band_style=b1.evaluate("e=>({align:getComputedStyle(e).alignItems})")
    label_style=b1.locator('.slot-map-band-label').evaluate("e=>({pt:getComputedStyle(e).paddingTop})")
    check(band_style['align']=='baseline',f'grid must use baseline alignment: {band_style}')
    check(label_style['pt']=='0px',f'Target local top offset must be removed: {label_style}')
    ...
    for width in (390,360):
        ...
        if width==390:
            ...
            page.screenshot(path=str(ROOT/'tests/v32-6-1-map-top.png'))
            ...
            page.wait_for_timeout(20); page.screenshot(path=str(ROOT/'tests/v32-6-1-map-s4.png'))

    check(not errors,f'page errors after interactions: {errors}')
    print('v32.6.1 runtime Playwright checks: OK')
    browser.close()
