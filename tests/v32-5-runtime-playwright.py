...
if 'fantacalcio-checklist-v32.6' in sw_text:
    print('v32.5 runtime checks superseded by v32.6 typographic alignment: OK')
    raise SystemExit(0)
...
with sync_playwright() as pw:
    ...
    s3_target_left=page.locator('[data-slot-map-slot="S3"] .slot-map-band-label').nth(0).evaluate('e=>e.getBoundingClientRect().left')
    check(abs(inline_left-s3_target_left)<=1.5,f'S1/S2 optical inset differs from Target grid: {inline_left}/{s3_target_left}')
    ...
    for width in (390,360):
        ...
        if width==390:
            ...
            page.screenshot(path=str(ROOT/'tests/v32-5-map-top.png'))
            ...
            page.wait_for_timeout(20); page.screenshot(path=str(ROOT/'tests/v32-5-map-s4.png'))

    check(not errors,f'page errors after interactions: {errors}')
    print('v32.5 runtime Playwright checks: OK')
    browser.close()
